          ...
          {pricingPlans.map((plan) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <h3
                    className={`text-2xl font-black ${isPopular ? "text-white" : "text-[#111114]"}`}
                  >
                    {plan.name}
                  </h3>

                  <p
                    className={`mt-2 text-sm leading-6 ${isPopular ? "text-white/75" : "text-[#667085]"}`}
                  >
                    {plan.tagline}
                  </p>
                </div>
                ...
                <div className="my-8">
                  <div className="flex items-end gap-1">
                    <span
                      className={`text-5xl font-black ${isPopular ? "text-white" : "text-[#111114]"}`}
                    >
                      ${finalPrice}
                    </span>

                    <span
                      className={`mb-2 text-sm ${isPopular ? "text-white/70" : "text-[#98A2B3]"}`}
                    >
                      /month
                    </span>
                  </div>
                  ...
                  <p
                    className={`mt-2 text-xs ${isPopular ? "text-white/70" : "text-[#98A2B3]"}`}
                  >
                    {billingCycle === "annually"
                  ...
                  className={`
                    ...
                    ${
                      isPopular
                        ...
                        `
                        : `
                          overflow-hidden group text-white bg-linear-to-r from-[#2587FF] to-[#8B3DFF] shadow-lg shadow-[#2587FF]/20 hover:shadow-[#8B3DFF]/30 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer
                        `
                    }
                  `}
                >
                  <span className="relative z-10">Start This Plan</span>
                ...
                {/* Divider */}
                <div
                  className={`my-8 h-px ${isPopular ? "bg-white/15" : "bg-[#EDF2FA]"}`}
                />
